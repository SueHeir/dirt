...
- **Preparation dynamics.** The dense but non-overlapping source is settled with
  ...
  first dense Hertz contacts without the solver's large-overlap abort. The released stage retains
  that timestep because a trajectory that runs at a larger timestep is not, by itself,
  timestep-convergence evidence for the runout observable. The longer 0.8 s
  ...
  part of the measured material model or a route to changing the runout gate.
