#!/usr/bin/env python3
"""
SPH glass-sphere calibration — column-collapse MACRO VALIDATION driver.

This is the end-to-end macro gate of the calibration: the calibrated material
(and, later, the SPH closure) must reproduce the experimental column-collapse
runout scaling. It is NOT a fitted closure parameter.

Releases a quasi-2D rectangular column of the canonical glass grains (initial
width L0, height H) on a flat floor for a range of aspect ratios a = H/L0, then
extracts the final runout L_f from each settled deposit and checks the
dimensionless runout against the experimental aspect-ratio scaling laws
(Lube et al. 2004; Lajeunesse et al. 2004):
...
Commands (from anywhere):
    ...
    python3 examples/SPH_glass_sphere_calibration/07_column_collapse/sweep.py   # all three
...
granular hertz/material ... tangential mindlin ... rolling sds ... damping tsuji,
same E/nu/e/mu/mu_r, gravity, and frictional floor + back + side + removable-gate
walls via fix wall/gran). LAMMPS's final deposit is parsed into the SAME
(x, y, z, radius) form and runout is extracted with the SAME measure_column() the
DIRT leg uses, so the two codes are compared on equal footing and overlaid (open
markers) on plots/runout_scaling.png. LAMMPS is optional: with no binary present,
only DIRT runs and the validation (DIRT-vs-theory) is unchanged.
...
SWEEP_DIR = os.path.join(SCRIPT_DIR, "sweep" + _TAG)
DATA_DIR = os.path.join(SCRIPT_DIR, "data")
PLOT_DIR = os.path.join(SCRIPT_DIR, "plots")
RUNOUT_CSV = os.path.join(DATA_DIR, f"runout{_TAG}.csv")   # DIRT runout per aspect
...
# Canonical density = 2500.0 kg/m^3 is set on the particle insert (below);
# DIRT carries grain density there, not on [[dem.materials]].
...
def generate():
    ...
    # Polydispersity: canonical ±10% by default (matches the shear rheometer);
    # POLY=<frac> overrides, POLY=0 forces the monodisperse legacy baseline.
    _f = float(os.environ.get("POLY", "0.10"))
    if _f > 0:
        rspec = (f'{{ distribution = "uniform", '
                 f'min = {(1 - _f) * RADIUS:.6e}, max = {(1 + _f) * RADIUS:.6e} }}')
    else:
        rspec = f"{RADIUS}"
    for a in ASPECTS:
        cdir = case_dir(a)
...
#   rolling sds {kr} {damp} {mu_r}             -> spring-dashpot-slider rolling
#       resistance with Coulomb cap {mu_r} (DIRT's rolling_friction analogue).
...
pair_style      granular
pair_coeff      1 1 hertz/material {E} {e} {nu} tangential mindlin NULL {tdamp} {mu} rolling sds {kr} {tdamp} {mu_r} damping tsuji twisting none

fix             grav all gravity {g} vector 0 0 -1
fix             floor all wall/gran granular hertz/material {E} {e} {nu} tangential mindlin NULL {tdamp} {mu} rolling sds {kr} {tdamp} {mu_r} damping tsuji twisting none zplane 0.0 NULL
fix             back all wall/gran granular hertz/material {E} {e} {nu} tangential mindlin NULL {tdamp} {mu} rolling sds {kr} {tdamp} {mu_r} damping tsuji twisting none xplane 0.0 NULL
fix             sides all wall/gran granular hertz/material {E} {e} {nu} tangential mindlin NULL {tdamp} {mu} rolling sds {kr} {tdamp} {mu_r} damping tsuji twisting none yplane 0.0 {W}
fix             gate all wall/gran granular hertz/material {E} {e} {nu} tangential mindlin NULL {tdamp} {mu} rolling sds {kr} {tdamp} {mu_r} damping tsuji twisting none xplane NULL {L0}
...
def write_lammps_input(path, aspect):
    ...
    # Rolling stiffness for the sds model. LAMMPS 'rolling sds' takes an absolute
    # k_roll; tie it to the tangential spring scale via the friction ratio so the
    # rolling cap mu_r is the dominant control (matches DIRT's rolling Coulomb cap).
    k_roll = 1.0e5
    with open(path, "w") as f:
        f.write(LMP_TEMPLATE.format(
            ...
            tdamp=1.0, mu=FRICTION, kr=f"{k_roll:.6e}", mu_r=ROLLING_FRICTION,
            g=9.81,
...
def validate(rows):
    print("=" * 66)
    print("Column-Collapse Runout — MACRO VALIDATION (SPH glass calibration)")
    print("=" * 66)
