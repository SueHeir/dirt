//! sphcal_shear_rheology — Lees–Edwards homogeneous simple-shear rheometer.
//!
//! Tier-1 of the SPH glass-sphere calibration campaign: a triperiodic box of
//! glass beads is sheared at a constant rate γ̇ via the native Lees–Edwards
...
//! `Φ` to `<output_dir>/data/sphcal_shear_rheology_results.csv`. `sweep.py`
//! time-averages the steady window, forms `I, μ, Φ`, and fits the μ(I) law.
//!
//! ```bash
//! cargo run --release --example sphcal_shear_rheology --no-default-features -- examples/SPH_glass_sphere_calibration/01_shear_rheology/config.toml
//! ```
...
fn record_shear(
    ...
    let out_dir = input
        .output_dir
        .clone()
        .unwrap_or_else(|| "examples/SPH_glass_sphere_calibration/01_shear_rheology/data".to_string());
    let path = format!("{}/sphcal_shear_rheology_results.csv", out_dir);

    static INIT: Once = Once::new();
    INIT.call_once(|| {
        fs::create_dir_all(&out_dir).ok();
        let mut f = fs::File::create(&path).expect("cannot create sphcal_shear_rheology_results.csv");
        writeln!(
            f,
