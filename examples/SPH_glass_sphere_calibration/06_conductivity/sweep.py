#!/usr/bin/env python3
"""
Granular-temperature conductivity κ(Φ) from a gas-free vibro-fluidized bed.

A bed of canonical glass beads on an oscillating base wall (periodic x,z, gravity
in −y) reaches a steady fluidized state with profiles Φ(y), T(y). The base injects
fluctuation energy that conducts upward and is dissipated inelastically +
frictionally, so in steady state (no mean shear) the energy balance is pure
conduction vs dissipation:
...
κ(y) = q_y(y) / (−dT/dy) is the conductivity closure for the granular-temperature
SPH de-fluidization energy balance. Because Φ varies with height, one run sweeps
**κ(Φ)**, compared to the KT (Lun/Gidaspow) conductivity. The measured kinetic heat
flux q_y(y) (recorded directly) is overlaid as an independent cross-check. With
friction (μ_p=0.16) the extra dissipation steepens the T(y) gradient enough to
measure κ.
...
makes the gradient measurable) but is not in the KT closure, so the energy-balance
κ* is an *effective* conductivity that lumps the friction-steepened gradient into
the closure — exactly the quantity the SPH energy balance needs. The KT curve is a
reference, not a strict PASS/FAIL target.
"""
...
def graph():
    ...
    fig.suptitle("Vibro-fluidized bed: steady profiles (canonical glass, μ_p=0.16)")
