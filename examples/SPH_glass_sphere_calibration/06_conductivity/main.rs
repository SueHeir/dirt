//! sphcal_conductivity — granular-temperature conductivity κ(Φ) for the
//! de-fluidization energy balance, from a gas-free vibro-fluidized bed.
//!
//! A bed of canonical glass beads sits under gravity on an **oscillating base
//! wall** (periodic in x,z). The vibrating base injects fluctuation (granular-
//! temperature) energy at the bottom; it conducts upward and is removed by
//! inelastic + frictional dissipation, producing steady **Φ(y)** and **T(y)**
//! profiles. With friction (μ_p = 0.16) the extra dissipation steepens the T(y)
//! gradient enough to measure κ — a frictionless near-elastic bed stays
//! isothermal. From the steady profiles `sweep.py` extracts the conductivity
//! **κ(Φ)** via the conduction–dissipation balance κ(y) = q_y(y) / (−dT/dy),
//! the conductivity closure for the granular-temperature SPH de-fluidization
//! energy balance.
//!
...
fn record_profile(
    ...
    let out_dir = input
        .output_dir
        .clone()
        .unwrap_or_else(|| "examples/SPH_glass_sphere_calibration/06_conductivity/data".to_string());
