...
//!
//! Protocol — GENTLE / quasi-static "lift the cylinder" (heap on a frictional
//! floor), at the REAL glass restitution e=0.926:
//!   1. Fill: particles are inserted inside a thin z-aligned cylinder wall that
//!      sits on the floor, and settle into a packed column under gravity. A
//!      `[[viscous]]` fix (F=−γv, ZERO at rest) bleeds the elastic grains'
//!      translational kinetic energy so the column settles WITHOUT changing the
//!      contact restitution — the static cone is the true e=0.926 friction-
//!      controlled repose. Viscous drag is translational only, so it does not
//!      touch the rolling DOF: rolling friction still sets the cone slope.
//!   2. Lift: once the column's kinetic energy drops below a threshold, the
//!      confining cylinder wall is deactivated (the "lift"). With the strong
//!      viscous drag the released column drains QUASI-STATICALLY (it cannot
//!      accelerate into a fast sliding avalanche), so grains ROLL into place and
//!      μ_r engages — the cone is rolling-friction-controlled, not slip-limited.
...
//! walls only), so the "lift" is the runtime deactivation below; the GENTLE pour
//! comes from the viscous drag, not a slowly-raised wall.
//!
...
fn main() {
    ...
    app.add_plugins(CorePlugins)
        ...
        .add_plugins(FixesPlugin) // [[viscous]] drag: gentle settle + quasi-static drain
    ...
    // Stage 2a: guarantee the cylinder is lifted on entering the lift stage.
    // The fill→lift run-stage boundary auto-advances Phase even if the strict
    // vmax settle threshold was not crossed in time; this fallback removes the
    // cylinder (and stamps lift_step) so the protocol always proceeds.
    app.add_update_system(
        ensure_cylinder_lifted.run_if(in_state(Phase::Lift)),
        ParticleSimScheduleSet::PostFinalIntegration,
    );
    // Stage 2b: wait for the heap to come to rest, then dump positions.
    app.add_update_system(
...
/// Fallback lift: if the run reached the lift stage without `lift_when_settled`
/// having fired (the column was still creeping above the strict vmax threshold
/// when the fill stage ended), deactivate the confining cylinder now and stamp
/// `lift_step`. Runs once; thereafter `lift_step` is `Some` and this is a no-op.
fn ensure_cylinder_lifted(
    run_state: Res<RunState>,
    comm: Res<CommResource>,
    mut walls: ResMut<Walls>,
    mut tracker: ResMut<ReposeTracker>,
) {
    if tracker.lift_step.is_some() {
        return;
    }
    let step = run_state.total_cycle;
    walls.deactivate_by_name("cylinder");
    ...
    if comm.rank() == 0 {
        println!(
            "Step {}: lift stage reached — deactivating cylinder (fallback lift)",
            step
        );
    }
}
