...
Commands:
    ...
    python3 examples/bench_granular_conductivity/sweep.py smoke   # same as no-arg: fast κ-vs-KT gate on config.smoke.toml
    python3 examples/bench_granular_conductivity/sweep.py full    # full scientific run (config.toml): profiles + κ(Φ)
    python3 examples/bench_granular_conductivity/sweep.py start   # build + run the full config only
    python3 examples/bench_granular_conductivity/sweep.py graph   # profiles + κ(Φ) from the full run
...
def measure_kappa(ys, phi, T, qy):
    ...
    conduction–dissipation balance (same math as graph()). Returns parallel lists
    (phis, k_eb, k_kin, k_kt) over the bins with a resolvable downward T-gradient."""
    n = len(ys)
    if n < 3:
        return [], [], [], []
    dy = ys[1] - ys[0]
...
def smoke():
    ...
    (NaN, wrong sign, order-of-magnitude error) fast; the tight κ*(Φ)-vs-KT
    comparison lives in the full run (`sweep.py full`, config.toml) unchanged.

    Reports 'ALL CHECKS PASSED' / 'CHECKS FAILED' and exits 0/1 for the harness."""
    # Physically-motivated smoke bounds (deterministic single-process run):
    ...
    checks = []
    n_ok = len(phis) >= MIN_BINS
    checks.append((n_ok, f"resolvable T-gradient bins: {len(phis)} (need ≥{MIN_BINS})"))
