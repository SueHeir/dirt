# BPM cantilever test — `[[freeze]]` rigid anchor

A 10-sphere bonded chain anchored at one end by `[[freeze]]` bends under its
own weight and settles toward the uniform-load cantilever deflection predicted
by beam theory. This exercises the exact failure mode the user's team hit:
a frozen anchor whose neighbour is loaded transversely — bond shear
produces a lever-arm torque that (with wrong sign conventions) created
explosive feedback.

Fixes that make this example pass:

1. **`[[freeze]]`** — full immobilization. Every step at `PostForce` it zeros
   `vel, force` (on `Atom`) AND `omega, torque, ang_mom` (on `DemAtom`). With
   velocity held at zero the anchor never drifts; with rotation zeroed it never
   spins. The rotation part is essential: without it, bond shear on the anchor's
   neighbour creates a lever-arm torque that rotates the anchor, which creates
   more shear velocity → positive feedback (and the whole chain swings about a
   free-spinning anchor). *(For a translation-only constraint that leaves
   rotation free — restoring a captured position every step — SOIL's `[[pin]]`
   fix is the counterpart.)*
...
3. **Extended `ghost_cutoff`** (from the MPI fix) — unrelated to the anchor
   blowup but still needed for bonds that span rank boundaries.

## Setup

- 10 glass-like spheres, radius 1 mm, spaced 2 mm on the x-axis
- Left end (tag 0) at `x = 0` in the `"anchor"` group, immobilized via `[[freeze]]`
