#!/usr/bin/env python3
"""
Haff's-cooling benchmark — freely cooling granular gas of rough spheres.

A periodic box of frictional spheres is given a Maxwellian velocity field and
left to cool through inelastic collisions. DIRT's contact has a velocity-
independent restitution (constant e), so the granular temperature must follow
Haff's law:

    T(t) = T0 / (1 + t/tc)^2          (late-time log-log slope = -2)

The same gas is also run in LAMMPS (matched normal + tangential contact) as an
independent cross-check. A 800-particle gas is chaotic, so the two codes are NOT
compared trajectory-by-trajectory — only the cooling law (the -2 slope and the
Haff fit).

Commands (from anywhere):
    python3 examples/bench_sphere_haff_cooling/sweep.py generate   # write LAMMPS input
    python3 examples/bench_sphere_haff_cooling/sweep.py start      # build + run DIRT and LAMMPS
    python3 examples/bench_sphere_haff_cooling/sweep.py graph      # validate vs Haff + plot
    python3 examples/bench_sphere_haff_cooling/sweep.py            # all three, in order

Outputs:
    data/in.lammps             generated LAMMPS input   (gitignored)
...
EXAMPLE = "bench_sphere_haff_cooling"
...
# Physics — must match config.toml.
N = 800
L = 0.04             # box side [m]
RADIUS = 0.0011      # m
...
STEPS = 700_000
OUTPUT_EVERY = 2000  # matches the example's measure_cooling cadence
...
def validate(dirt):
    ...
    print("=" * 60)
    print("Haff Cooling Validation — Single Rough Spheres (DIRT)")
    print("=" * 60)
    ...
    # Kinetic-theory cooling time (translational, Carnahan-Starling g0).
    n = N / L**3
    phi = n * (4.0 / 3.0) * math.pi * RADIUS**3
    g0 = (1.0 - phi / 2.0) / (1.0 - phi) ** 3
    omega0 = (4.0 / 3.0) * n * (2 * RADIUS)**2 * g0 * math.sqrt(math.pi) * (1 - RESTITUTION**2)
    tc_theory = 2.0 / (omega0 * math.sqrt(Tt[0]))
    print(f"\n  Haff fit:  T0={T0:.3e}  tc={tc:.3e} s")
    print(f"  kinetic theory:  phi={phi:.3f}  g0={g0:.3f}  tc_theory={tc_theory:.3e} s"
          f"  (fit/theory={tc/tc_theory:.2f})")
...
def graph():
    ...
    ax.set_title("Cooling law: DIRT vs LAMMPS")
    ...
    ax.set_title("DIRT energy partition (rough spheres)")
    ax.legend(fontsize=8)

    fig.suptitle("Haff Cooling — Single Rough Spheres", y=1.02)
    fig.tight_layout()
