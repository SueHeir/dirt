#!/usr/bin/env python3
"""
Haff's-cooling benchmark — rod clumps (4 spheres in a line).

A periodic box of rigid rod-shaped clumps is given a random velocity field and
left to cool through inelastic collisions. DIRT's contact has a velocity-
independent restitution (constant e), so the granular temperature obeys Haff's
law:

    T(t) = T0 / (1 + t/tc)^2          (late-time log-log slope -> -2)

The rod's highly asymmetric inertia tensor (Ix << Iy ~ Iz) exercises the
angular-momentum integration. The same gas is also run in LAMMPS (rigid
multisphere via `fix rigid/small`, matched Hertz + Mindlin + tsuji contact) as an
independent cross-check. A many-body granular gas is chaotic and the two codes
use different rigid integrators / clump-contact handling, so the comparison is on
the *cooling law* (Haff fit / -2 slope), not trajectory-by-trajectory.

Commands (from anywhere):
    python3 examples/bench_rod_haff_cooling/sweep.py generate   # write rod.mol + in.lammps
    python3 examples/bench_rod_haff_cooling/sweep.py start      # build + run DIRT and LAMMPS
    python3 examples/bench_rod_haff_cooling/sweep.py graph      # validate vs Haff + plot
    python3 examples/bench_rod_haff_cooling/sweep.py            # all three, in order

Outputs (all under data/, gitignored, except the tracked figure):
    data/rod.mol, data/in.lammps, data/haff_trace.txt, data/lammps_cooling.csv
...
EXAMPLE = "bench_rod_haff_cooling"
TITLE = "Rod Clumps (4-sphere)"

DATA_DIR = os.path.join(SCRIPT_DIR, "data")
CONFIG = os.path.join(SCRIPT_DIR, "config.toml")
DIRT_CSV = os.path.join(DATA_DIR, "cooling.csv")
MOL_FILE = os.path.join(DATA_DIR, "rod.mol")
...
# rod4: 4 sub-spheres along x at -1.2, -0.4, 0.4, 1.2 mm.
SUB_R = 0.0005
CLUMP = [(-0.0012, 0.0, 0.0), (-0.0004, 0.0, 0.0),
         (0.0004, 0.0, 0.0), (0.0012, 0.0, 0.0)]
N = 500
L = 0.04
DENSITY = 2500.0
YOUNGS_MOD = 5.0e7
...
def start():
    ...
    print(f"Running DIRT ({N} rods, {STEPS} steps)...", flush=True)
...
def graph():
    ...
    ax.set_title("DIRT energy partition (asymmetric inertia)")
    ax.legend(fontsize=8)
