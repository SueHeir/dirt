//! bench_twisting_friction — validates the twisting- (torsional-) friction
//! torque model against the exact analytical spin-down of a sphere twisting on
//! an enduring contact.
//!
//! ## Setup
...
//! rolling — the ONLY contact torque is the twisting-friction couple, applied
//! purely along n̂. The spin therefore decays about a fixed axis at a constant
//! (constant-model) or piecewise-constant (SDS, after a brief elastic wind-up)
//! rate, while ω_x, ω_y and the horizontal position stay at zero.
//!
//! ## Analytical spin-down
//! For DIRT's twisting couple `τ_tw = μ_tw · F_n · r_eff` opposing the twist,
//! with `F_n = m g`, equal-sphere `r_eff = R/2`, and `I = (2/5) m R²`:
//!
//!     α = dω/dt = − τ_tw / I = − (5/4) · μ_tw · g / R      (exact, constant)
//!
//! The SDS spring-dashpot-slider model winds a torsional spring up to the same
//! cap `τ_max = μ_tw F_n r_eff` within a few steps and then holds it (the twist
//! keeps growing, so the slider stays saturated), giving the identical constant
//! deceleration in the saturated regime — `sweep.py` fits the slope past the
//! wind-up transient.
//!
...
//!
//! ```bash
//! cargo run --release --example bench_twisting_friction --no-default-features --features precision-double -- examples/bench_twisting_friction/config.toml
//! ```
...
/// slowest-μ_tw case still spins down within the configured step budget.
...
/// across the sweep (only μ_tw and the twisting model change), so this seating
/// stays exact for every case.
...
fn record(
    ...
    if halted || step + 1 >= last_step {
        let out_dir = input
            .output_dir
            .clone()
            .unwrap_or_else(|| "examples/bench_twisting_friction".to_string());
        ...
        println!("=== Twisting Friction Results ===");
