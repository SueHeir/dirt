...
overlaps and records the normal force. `sweep.py` compares the trace to an
independent analytic implementation of the documented one-dimensional MDR subset:
elastic loading, first yield, plastic unloading, MDR contact-radius stiffness, and
the adhesive tensile branch. The gate is `2e-10` relative or `1e-8 N` absolute.

![MDR normal force trace](bench_mdr_elastoplastic_normal/plots/mdr_force_trace.png)

Current result: PASS; maximum relative error `3.801e-13`.
...
| mdr_elastoplastic_normal | one-dimensional MDR loading/yield/plastic-unloading trace | analytical (subset) | PASS; max relative error 3.801e-13 vs independent reference; full LAMMPS apparent-radius/free-surface MDR state intentionally not included |
