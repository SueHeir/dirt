...
pub fn print_granular_temperature(
    ...
    for i in 0..nlocal {
        local_mv_x += atoms.mass[i] as f64 * atoms.vel[i][0] as f64;
        local_mv_y += atoms.mass[i] as f64 * atoms.vel[i][1] as f64;
        local_mv_z += atoms.mass[i] as f64 * atoms.vel[i][2] as f64;
        local_mass += atoms.mass[i] as f64;
    }
    ...
    for i in 0..nlocal {
        vel_diff += atoms.mass[i] as f64 * (atoms.vel[i][0] as f64 - avg_vx).powi(2)
            + atoms.mass[i] as f64 * (atoms.vel[i][1] as f64 - avg_vy).powi(2)
            + atoms.mass[i] as f64 * (atoms.vel[i][2] as f64 - avg_vz).powi(2);
    }
