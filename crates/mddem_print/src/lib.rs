use std::{fs::{self, File, OpenOptions}, io::{Write, BufWriter}, time::Instant};

use serde::{Serialize, Deserialize};
use mddem_app::prelude::*;
use mddem_scheduler::prelude::*;

use mddem_core::{Config, Atom, AtomDataRegistry, CommResource, Input, RunConfig, RunState};
...
impl Thermo {
    pub fn new() -> Self {
        Thermo { interval: 100, start_time: Instant::now(), last_printed_step: 0 }
    }
}

// ── Dump config ─────────────────────────────────────────────────────────────

fn default_dump_format() -> String { "text".to_string() }
...
impl Default for DumpConfig {
    fn default() -> Self {
        DumpConfig { interval: 0, format: "text".to_string() }
    }
}

// ── Restart config ──────────────────────────────────────────────────────────

fn default_restart_format() -> String { "bincode".to_string() }
...
impl Default for RestartConfig {
    fn default() -> Self {
        RestartConfig { interval: 0, format: "bincode".to_string(), read: false }
    }
}
...
struct RestartData {
    ...
    omega_x: Vec<f64>, omega_y: Vec<f64>, omega_z: Vec<f64>,
    force_x: Vec<f64>, force_y: Vec<f64>, force_z: Vec<f64>,
    torque_x: Vec<f64>, torque_y: Vec<f64>, torque_z: Vec<f64>,
    ang_mom_x: Vec<f64>, ang_mom_y: Vec<f64>, ang_mom_z: Vec<f64>,
...
impl Default for VtpConfig {
    fn default() -> Self {
        VtpConfig { interval: 0 }
    }
}
...
pub fn setup_thermo(config: Res<RunConfig>, scheduler_manager: Res<SchedulerManager>, comm: Res<CommResource>, run_state: Res<RunState>, mut thermo: ResMut<Thermo>) {
    ...
    if comm.rank() == 0 {
        println!();
        println!("{:<12} {:<8} {:<14} {:<12} {:<14} {:<12}", "Step", "Atoms", "KE(J)", "Neighbors", "WallTime(s)", "Step/s");
        println!("{}", "-".repeat(74));
    }
}

pub fn print_thermo(atoms: Res<Atom>, run_state: Res<RunState>, comm: Res<CommResource>, neighbor: Res<Neighbor>, mut thermo: ResMut<Thermo>) {
    let step = run_state.total_cycle;
    if step % thermo.interval != 0 { return; }
    let nlocal = atoms.nlocal as usize;
    let local_ke: f64 = (0..nlocal).map(|i| {
        let vx = atoms.vel_x[i]; let vy = atoms.vel_y[i]; let vz = atoms.vel_z[i];
    ...
    if comm.rank() == 0 {
        ...
        println!("{:<12} {:<8} {:<14.6e} {:<12} {:<14.4} {:<12.1}", step, atoms.natoms, global_ke, global_neighbors, elapsed, steps_per_sec);
        thermo.start_time = Instant::now();
...
pub fn print_vtp(atoms: Res<Atom>, run_state: Res<RunState>, comm: Res<CommResource>, input: Res<Input>, vtp_config: Res<VtpConfig>, run_config: Res<RunConfig>, scheduler_manager: Res<SchedulerManager>) {
    ...
    if let Err(_error) = result { println!("Could not create file directory {}", base_dir) }
    ...
    for i in 0..atoms.len() { writeln!(&mut file, "{} {} {}", atoms.pos_x[i], atoms.pos_y[i], atoms.pos_z[i]).unwrap(); }
    ...
    for i in 0..atoms.len() { writeln!(&mut file, "{}", atoms.skin[i]).unwrap(); }
    write!(&mut file, "</DataArray>").unwrap();
    write!(&mut file, "<DataArray type=\"Float32\" Name=\"Vel_Mag\" format=\"ascii\">\n").unwrap();
    for i in 0..atoms.len() {
        let vmag = (atoms.vel_x[i].powi(2) + atoms.vel_y[i].powi(2) + atoms.vel_z[i].powi(2)).sqrt();
        writeln!(&mut file, "{}", vmag).unwrap();
    }
    write!(&mut file, "</DataArray>").unwrap();
    write!(&mut file, "<DataArray type=\"Int32\" Name=\"IsGhost\" format=\"ascii\">\n").unwrap();
    for i in 0..atoms.len() { let is_ghost = if i >= atoms.nlocal as usize { 1 } else { 0 }; writeln!(&mut file, "{}", is_ghost).unwrap(); }
    write!(&mut file, "</DataArray>").unwrap();
    write!(&mut file, "<DataArray type=\"Int32\" Name=\"IsCollision\" format=\"ascii\">\n").unwrap();
    for i in 0..atoms.len() { let is_collision = if atoms.is_collision[i] { 1 } else { 0 }; writeln!(&mut file, "{}", is_collision).unwrap(); }
    write!(&mut file, "</DataArray>").unwrap();
...
pub fn print_granular_temperature(atoms: Res<Atom>, run_state: Res<RunState>, comm: Res<CommResource>, thermo: Res<Thermo>, input: Res<Input>) {
    if run_state.total_cycle % thermo.interval != 0 { return; }
    let nlocal = atoms.nlocal as usize;
    ...
    let base_dir = match input.output_dir.as_deref() { Some(dir) => format!("./{}/data", dir), None => "./data".to_string() };
    let result = fs::create_dir_all(&base_dir);
    if let Err(_error) = result { println!("Could not create file directory {}", base_dir) }
    let data_path = format!("{}/GranularTemp.txt", base_dir);
    let mut file = if run_state.total_cycle == 0 {
        OpenOptions::new().create(true).write(true).truncate(true).open(&data_path).unwrap()
    } else {
        OpenOptions::new().create(true).append(true).open(&data_path).unwrap()
    };
    write!(&mut file, "{} {:.6e} {:.10e}\n", run_state.total_cycle, physical_time, granular_temperature).unwrap();
}
    ...
    match dump_config.format.as_str() {
        ...
        _ => {
            ...
            for i in 0..nlocal {
                let radius = dem.as_ref().map(|d| d.radius[i]).unwrap_or(atoms.skin[i]);
                writeln!(w, "{},{},{},{},{},{},{},{},{},{},{},{}",
                    atoms.tag[i], atoms.atom_type[i],
                    atoms.pos_x[i], atoms.pos_y[i], atoms.pos_z[i],
                    atoms.vel_x[i], atoms.vel_y[i], atoms.vel_z[i],
                    atoms.force_x[i], atoms.force_y[i], atoms.force_z[i],
                    radius,
                ).unwrap();
            }
        }
...
pub fn write_restart(
    ...
    let data = RestartData {
        ...
        quaternion: (0..nlocal).map(|i| {
            let q = atoms.quaterion[i];
            [q.w, q.i, q.j, q.k]
        }).collect(),
        mass: atoms.mass[..nlocal].to_vec(),
        skin: atoms.skin[..nlocal].to_vec(),
        radius: dem.as_ref().map(|d| d.radius[..nlocal].to_vec()).unwrap_or_default(),
        density: dem.as_ref().map(|d| d.density[..nlocal].to_vec()).unwrap_or_default(),
    };
...
pub fn read_restart(
    ...
    let step = match latest_step {
        Some(s) => s,
        None => {
            if comm.rank() == 0 { println!("Restart: no restart files found in {}", base_dir); }
            return;
        }
    };

    let filename = format!("{}/restart_{}_rank{}.{}", base_dir, step, rank, ext);
    if comm.rank() == 0 { println!("Restart: reading from {}", filename); }
    ...
    atoms.pos_x = data.pos_x; atoms.pos_y = data.pos_y; atoms.pos_z = data.pos_z;
    atoms.vel_x = data.vel_x; atoms.vel_y = data.vel_y; atoms.vel_z = data.vel_z;
    atoms.omega_x = data.omega_x; atoms.omega_y = data.omega_y; atoms.omega_z = data.omega_z;
    atoms.force_x = data.force_x; atoms.force_y = data.force_y; atoms.force_z = data.force_z;
    atoms.torque_x = data.torque_x; atoms.torque_y = data.torque_y; atoms.torque_z = data.torque_z;
    atoms.ang_mom_x = data.ang_mom_x; atoms.ang_mom_y = data.ang_mom_y; atoms.ang_mom_z = data.ang_mom_z;
    atoms.quaterion = data.quaternion.iter().map(|q| {
