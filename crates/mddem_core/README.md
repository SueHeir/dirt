...
- **`input`** — TOML config loading via `Config` resource. `Config::load::<T>(app, key)` deserializes a TOML section and adds it as an `App` resource. `InputPlugin` handles CLI parsing and banner printing.
